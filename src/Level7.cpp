...
void App::Level_7_Start() {
    ...
    m_Canvas_7 = std::make_shared<Character>(RESOURCE_DIR"/Level/Level_7.png"); // left = 435 / right = 835
    m_Canvas_7->SetPosition({0, 0}); //x - to left + to right // - down + up // -205.0f & 205.0f
    m_Canvas_7->SetZIndex(95);
    m_Root.AddChild(m_Canvas_7);
    ...
    m_CurrentState = State::LEVEL_7_UPDATE;
}

void App::Level_7_Update() {
    //exit
    if (Util::Input::IsKeyPressed(Util::Keycode::ESCAPE) || Util::Input::IfExit()) {
        m_GameStartSFX->Play();
        m_CurrentState = State::END;
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        if (!m_IsCheating) {
            if (m_Bird->IfCollides(m_PipeUp[i]) || m_Bird->IfCollides(m_PipeDown[i])) {
                ...
                m_CurrentState = State::LEVEL_7_GAMEOVER;
