...
void App::Level_5_Start() {
    ...
    m_Canvas_5 = std::make_shared<Character>(RESOURCE_DIR"/Level/Level_5.png"); // left = 435 / right = 835
    m_Canvas_5->SetPosition({0, 0}); //x - to left + to right // - down + up // -205.0f & 205.0f
    m_Canvas_5->SetZIndex(95);
    m_Root.AddChild(m_Canvas_5);
    ...
    m_CurrentState = State::LEVEL_5_UPDATE;
}

void App::Level_5_Update() {
    //exit
    if (Util::Input::IsKeyPressed(Util::Keycode::ESCAPE) || Util::Input::IfExit()) {
        m_GameStartSFX->Play();
        m_CurrentState = State::END;
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        if (!m_IsCheating) {
            if (m_Bird->IfCollides(m_PipeUp[i]) || m_Bird->IfCollides(m_PipeDown[i])) {
                ...
                m_CurrentState = State::LEVEL_5_GAMEOVER;
