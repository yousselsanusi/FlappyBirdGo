...
void App::Level_10_Start() {
    ...
    m_Canvas_10 = std::make_shared<Character>(RESOURCE_DIR"/Level/Level_10.png"); // left = 435 / right = 835
    m_Canvas_10->SetPosition({0, 0}); //x - to left + to right // - down + up // -205.0f & 205.0f
    m_Canvas_10->SetZIndex(95);
    m_Root.AddChild(m_Canvas_10);
    ...
    m_CurrentState = State::LEVEL_10_UPDATE;
}

void App::Level_10_Update() {
    //exit
    if (Util::Input::IsKeyPressed(Util::Keycode::ESCAPE) || Util::Input::IfExit()) {
        m_GameStartSFX->Play();
        m_CurrentState = State::END;
    ...
    if (m_IsWinning) {
        m_YouWin_3->SetVisible(true);
        m_Canvas_10->SetVisible(false);
        ...
        if (Util::Input::IsKeyPressed(Util::Keycode::R)) {
            ...
            m_IsCheating = false;
            m_CurrentState = State::LEVEL_10_START;
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        if (!m_IsCheating) {
            if (m_Bird->IfCollides(m_PipeUp[i]) || m_Bird->IfCollides(m_PipeDown[i])) {
                ...
                m_CurrentState = State::LEVEL_10_GAMEOVER;
