...
void App::Level_9_Start() {
    ...
    m_Canvas_9 = std::make_shared<Character>(RESOURCE_DIR"/Level/Level_9.png"); // left = 435 / right = 835
    m_Canvas_9->SetPosition({0, 0}); //x - to left + to right // - down + up // -205.0f & 205.0f
    m_Canvas_9->SetZIndex(95);
    m_Root.AddChild(m_Canvas_9);
    ...
    for (int i = 0; i < 20; ++i) {
        ...
        if (!m_IsCheating) {
            if (m_Bird->IfCollides(m_PipeUp[i]) || m_Bird->IfCollides(m_PipeDown[i])) {
                ...
                m_CurrentState = State::LEVEL_9_GAMEOVER;
